...
static qboolean	winsockInitialized = qfalse;
	...
	// PS4: use native sceNet API exclusively.
	//
	// Background: libxnet.a in the OpenOrbis toolchain is an empty 8-byte stub.
	// sendto.lo and bind.lo in libc.a are also empty objects with no code.
	// BSD compat socket functions are unresolved at link time and resolve at
	// runtime from the PS4 kernel's FreeBSD compat layer -- but that layer is
	// unreliable for UDP sendto (packets are silently discarded).
	//
	// All working PS4 homebrew (CTurt PS4-SDK, ps4-payload-sdk, debugnet, SM64)
	// use sceNetSocket/sceNetSendto/sceNetRecvfrom directly.  We do the same.
	//
	// IMPORTANT: never mix sceNet handles with BSD fcntl/ioctl/close -- those
	// take POSIX FDs.  We use sceNetSocketClose/sceNetSetsockopt exclusively.
#	include <orbis/Net.h>
#	include <orbis/libkernel.h>  /* sceKernelUsleep */
#	include <sys/socket.h>   /* struct sockaddr, struct sockaddr_storage, sa_family_t */
#	include <netinet/in.h>
#	include <arpa/inet.h>
#	include <netdb.h>        /* struct addrinfo (used in SearchAddrInfo) */
...
#	include <string.h>
	...
	// FreeBSD socket option constants (matching PS4 sceNet)
	// sys/socket.h from OpenOrbis already defines these correctly, but we
	// explicitly define them here so they're visible even without the header.
#	ifndef SOL_SOCKET
...
#	endif
	...
	// Non-blocking via socket timeouts (no FIONBIO in native sceNet).
	// SO_RCVTIMEO=1ms: recvfrom returns EAGAIN quickly when no data, keeping
	//   the game loop from blocking.
	// SO_SNDTIMEO=50ms: first sendto to a new unicast IP triggers an ARP lookup;
	//   without a send timeout the kernel returns EAGAIN=35 immediately while ARP
	//   is in-flight.  50ms is enough for ARP to complete on a LAN.
	static inline int ps4_net_setnonblocking(OrbisNetId fd) {
		struct timeval rcv = {0, 1000};    // 1 ms receive timeout
		struct timeval snd = {0, 50000};   // 50 ms send timeout (ARP resolution)
		sceNetSetsockopt(fd, SOL_SOCKET, SO_RCVTIMEO, &rcv, sizeof(rcv));
		sceNetSetsockopt(fd, SOL_SOCKET, SO_SNDTIMEO, &snd, sizeof(snd));
		return 0;
	}

	// Redirect BSD socket calls to native sceNet
#	define socket(af, type, proto) \
		sceNetSocket("ioq3", (af), (type), (proto))
#	define bind(fd, addr, addrlen) \
		sceNetBind((fd), (const OrbisNetSockaddr*)(addr), (OrbisNetSocklen_t)(addrlen))
#	define sendto(fd, buf, len, flags, addr, addrlen) \
		sceNetSendto((fd), (buf), (size_t)(len), (flags), \
		             (const OrbisNetSockaddr*)(addr), (OrbisNetSocklen_t)(addrlen))
#	define recvfrom(fd, buf, len, flags, addr, addrlen) \
		sceNetRecvfrom((fd), (buf), (size_t)(len), (flags), \
		               (OrbisNetSockaddr*)(addr), (OrbisNetSocklen_t*)(addrlen))
#	define setsockopt(fd, lev, opt, val, len) \
		sceNetSetsockopt((fd), (lev), (opt), (val), (OrbisNetSocklen_t)(len))
#	define closesocket(fd)      sceNetSocketClose(fd)
#	define ioctlsocket(fd, cmd, val) ps4_net_setnonblocking(fd)

	// Bypass select(): sceNet handles aren't POSIX FDs, so select() would block
	// forever or fail. Instead, always return "ready" and let sceNetRecvfrom
	// (with SO_RCVTIMEO=1ms) handle the no-data case via EAGAIN.
	// The real fd_set/FD_SET/FD_ISSET from sys/select.h work correctly with
	// sceNet handle IDs (they're small integers well within FD_SETSIZE=1024).
#	define select(n, r, w, e, tv) (1)

typedef OrbisNetId SOCKET;
#	define INVALID_SOCKET		((OrbisNetId)-1)
...
static int ps4_netDebugSends;
static int ps4_netDebugReceives;
static int ps4_netDebugErrors;
static int ps4_sendPacketCalls;
#endif
...
#if defined(__ORBIS__) || defined(__PS4__)
		((struct sockaddr_in *)s)->sin_len = sizeof(struct sockaddr_in);
#endif
	}
...
*/
#if defined(__ORBIS__) || defined(__PS4__)
// PS4: prefer numeric addresses, then use libSceNet's resolver.
// OpenOrbis does not export gethostbyname in this link set.
static qboolean Sys_StringToSockaddr(const char *s, struct sockaddr *sadr, int sadr_len, sa_family_t family)
{
	...
	{
		struct sockaddr_in *addr4 = (struct sockaddr_in *)sadr;
		if ((int)sadr_len >= (int)sizeof(*addr4) && inet_pton(AF_INET, s, &addr4->sin_addr) == 1)
		{
	...
	if (family == AF_UNSPEC || family == AF_INET)
	{
		...
		extern int PS4_GetNetPoolId(void);
		OrbisNetId resolverid;
		int ret;

		if ((int)sadr_len >= (int)sizeof(*addr4))
		{
			resolverid = sceNetResolverCreate("ioq3dns", PS4_GetNetPoolId(), 0);
			if (resolverid >= 0) {
				memset(&resolved, 0, sizeof(resolved));
				ret = sceNetResolverStartNtoa(resolverid, s, &resolved, 3 * 1000 * 1000, 2, 0);
				sceNetResolverDestroy(resolverid);
				if (ret == 0) {
					addr4->sin_family = AF_INET;
					addr4->sin_addr.s_addr = resolved.s_addr;
					return qtrue;
				}
				Com_Printf("PS4 NET resolver failed for %s: ret=0x%08X pool=%d rid=%d\n",
					s, ret, PS4_GetNetPoolId(), resolverid);
			} else {
				Com_Printf("PS4 NET resolver create failed for %s: ret=0x%08X pool=%d\n",
					s, resolverid, PS4_GetNetPoolId());
			}
		}
	}
	...
	{
		struct sockaddr_in6 *addr6 = (struct sockaddr_in6 *)sadr;
		if ((int)sadr_len >= (int)sizeof(*addr6) && inet_pton(AF_INET6, s, &addr6->sin6_addr) == 1)
		{
	...
	Com_Printf("Sys_StringToSockaddr: unable to resolve %s\n", s);
	return qfalse;
}
#else
static qboolean Sys_StringToSockaddr(const char *s, struct sockaddr *sadr, int sadr_len, sa_family_t family)
...
static void Sys_SockaddrToString(char *dest, int destlen, struct sockaddr *input)
{
	...
	{
		struct sockaddr_in *in4 = (struct sockaddr_in *)input;
		if (!inet_ntop(AF_INET, &in4->sin_addr, dest, destlen) && destlen > 0)
			*dest = '\0';
	}
	else if (input->sa_family == AF_INET6)
	{
		struct sockaddr_in6 *in6 = (struct sockaddr_in6 *)input;
		if (!inet_ntop(AF_INET6, &in6->sin6_addr, dest, destlen) && destlen > 0)
			*dest = '\0';
	}
	else if (destlen > 0)
	{
...
qboolean NET_GetPacket(netadr_t *net_from, msg_t *net_message, fd_set *fdr)
{
	...
	if(ip_socket != INVALID_SOCKET && FD_ISSET(ip_socket, fdr))
	{
		...
		// sceNetRecvfrom returns PS4-specific negative codes, not exactly -1
		if (ret < 0)
		{
#if !defined(__ORBIS__) && !defined(__PS4__)
			err = socketError;
			if( err != EAGAIN && err != ECONNRESET && err != EWOULDBLOCK )
...
#if defined(__ORBIS__) || defined(__PS4__)
			if (ps4_netDebugReceives < 32) {
				netadr_t dbgfrom;
				SockadrToNetadr( (struct sockaddr *) &from, &dbgfrom );
				Com_Printf("PS4 NET recv %d bytes from %s\n", ret, NET_AdrToStringwPort(dbgfrom));
				ps4_netDebugReceives++;
			}
#endif
			return qtrue;
		}
...
void Sys_SendPacket( int length, const void *data, netadr_t to ) {
...
#if defined(__ORBIS__) || defined(__PS4__)
	// PS4 sceNet rejects 255.255.255.255 (INADDR_BROADCAST).
	// Replace with the actual subnet broadcast (e.g. 192.168.1.255).
	// Note: do NOT use addr.ss_family here -- OpenOrbis sockaddr_storage has
	// ss_family at offset 0 (Linux layout) but sockaddr_in has sin_len at
	// offset 0 (BSD layout), so addr.ss_family reads sin_len=16, not AF_INET.
	// Use to.type instead.
	if (to.type == NA_BROADCAST) {
		extern uint32_t PS4_GetSubnetBroadcast(void);
		uint32_t bcast = PS4_GetSubnetBroadcast();
		if (bcast)
			((struct sockaddr_in *)&addr)->sin_addr.s_addr = bcast;
	}
	ps4_sendPacketCalls++;
	if (ps4_sendPacketCalls <= 4) {
		struct sockaddr_in *s4 = (struct sockaddr_in *)&addr;
		Com_Printf("PS4 NET Sys_SendPacket #%d: fd=%d type=%d len=%d dest=%s port=%d\n",
			ps4_sendPacketCalls, ip_socket, to.type, length,
			inet_ntoa(s4->sin_addr), (int)ntohs(s4->sin_port));
	}
#endif
	...
	if( usingSocks && to.type == NA_IP ) {
		...
		*(short *)&socksBuf[8] = ((struct sockaddr_in *)&addr)->sin_port;
		...
		// Use to.type, not addr.ss_family: OpenOrbis sockaddr_storage.ss_family
		// is at offset 0 (Linux layout) but sin_len occupies offset 0 (BSD), so
		// addr.ss_family would read sin_len=16 instead of AF_INET=2.
	...
	if( ret < 0 ) {
		int err = socketError;
		if (ps4_netDebugErrors < 32) {
			Com_Printf("PS4 NET sendto FAIL: fd=%d ret=%d errno=%d to=%s len=%d\n",
				ip_socket, ret, err, NET_AdrToStringwPort(to), length);
			ps4_netDebugErrors++;
		}
		return; // non-fatal for UDP
	}
	if (ps4_netDebugSends < 32) {
		Com_Printf("PS4 NET sent %d bytes to %s\n", ret, NET_AdrToStringwPort(to));
		ps4_netDebugSends++;
	}
#else
	if( ret == SOCKET_ERROR ) {
...
SOCKET NET_IPSocket( char *net_interface, int port, int *err ) {
	...
	*err = 0;
	...
	// PS4: ioctlsocket macro calls ps4_net_setnonblocking (SO_RCVTIMEO=1ms)
	// Other: standard ioctl FIONBIO
...
void NET_OpenSocks( int port ) {
	// SOCKS proxy is not wired up on PS4 yet.
	Com_Printf("NET_OpenSocks: SOCKS proxy not supported on PS4\n");
	usingSocks = qfalse;
}
#else
void NET_OpenSocks( int port ) {
...
#if (defined(__linux__) || defined(__APPLE__) || defined(__BSD__)) && !defined(__ORBIS__) && !defined(__PS4__)
...
#elif defined(__ORBIS__) || defined(__PS4__)
extern uint32_t PS4_GetLocalIP(void);
extern uint32_t PS4_GetSubnetMask(void);
static void NET_GetLocalAddress(void) {
	uint32_t ip      = PS4_GetLocalIP();     // host byte order, cached from PS4_NetInit
	uint32_t netmask = PS4_GetSubnetMask();
	...
	if (ip) {
		...
		nm.sin_addr.s_addr = htonl(netmask);
		NET_AddLocalAddress("en0",
			(struct sockaddr *)&sa,
			(struct sockaddr *)&nm);
	}
	Com_Printf("NET_GetLocalAddress: PS4 sceNetCtl, %d address(es)\n", numIP);
	Sys_ShowIP();
}
#else
static void NET_GetLocalAddress( void ) {
...
#elif defined(__ORBIS__) || defined(__PS4__)
	// PS4 port currently uses IPv4 sceNet sockets only.
	net_enabled = Cvar_Get( "net_enabled", "1", CVAR_LATCH | CVAR_ARCHIVE );
	if (net_enabled->integer != NET_ENABLEV4) {
		Com_Printf("PS4 NET: forcing net_enabled from %d to IPv4-only\n", net_enabled->integer);
		Cvar_SetValue("net_enabled", NET_ENABLEV4);
		net_enabled->modified = qtrue;
	}
#else
...
void NET_Init( void ) {
#if defined(__ORBIS__) || defined(__PS4__)
	Com_Printf("PS4 NET: direct sceNet UDP path enabled\n");
#endif
#ifdef _WIN32
	int		r;
