...
#ifdef __WIIU__
	/* ANGLE/GX2: No FBO pipeline, rendering goes directly to default
	   framebuffer.  Must clear color+depth every frame unconditionally,
	   otherwise uninitialized framebuffer memory shows through.
	   The normal r_fastsky clear only fires for 3D views (not 2D menu). */
	qglClearColor( 0.0f, 0.0f, 0.2f, 1.0f );  /* dark blue to distinguish "cleared" from "drawn black" */
