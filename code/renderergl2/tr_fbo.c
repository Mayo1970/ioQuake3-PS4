...
void FBO_CreateBuffer(FBO_t *fbo, int format, int index, int multisample)
{
	...
	switch(format)
	{
		...
		/* On PS4/GLES2, 24/32-bit depth map to GL_DEPTH_COMPONENT16 (duplicate case) */
...
#if !defined(__ORBIS__) && !defined(__PS4__)
		/* On PS4/GLES2, all stencil variants map to GL_STENCIL_INDEX8 (duplicate case) */
...
#if defined(__ORBIS__) || defined(__PS4__)
		FBO_CreateBuffer(tr.renderFbo, GL_DEPTH_COMPONENT16, 0, 0);
#else
...
#if defined(__ORBIS__) || defined(__PS4__)
		// Piglet returns GL_FRAMEBUFFER_UNSUPPORTED for depth-texture attachments even with
		// GL_OES_depth_texture; use a renderbuffer instead (depth texture sampling is unused
		// anyway since readDepth=qfalse)
		FBO_CreateBuffer(tr.renderFbo, GL_DEPTH_COMPONENT16, 0, 0);
#else
...
#if defined(__ORBIS__) || defined(__PS4__)
		FBO_CreateBuffer(tr.screenScratchFbo, GL_DEPTH_COMPONENT16, 0, 0);
#else
...
#if defined(__ORBIS__) || defined(__PS4__)
		FBO_CreateBuffer(tr.renderCubeFbo, GL_DEPTH_COMPONENT16, 0, 0);
#else
