...
static void GLSL_GetShaderHeader( GLenum shaderType, const GLchar *extra, char *dest, int size )
{
	...
	// On GLES2 (qglesMajorVersion >= 2) Piglet reports GLSL ES 2.0 (major=2, minor=0)
	// which would trip this condition, but we target GLSL ES 1.00 (#version 100) which
	// uses attribute/varying/gl_FragColor — NOT the in/out/out_Color keywords added below.
	// Force GLES2 into the else branch which generates correct GLSL ES 1.00 syntax.
	...
	else
	{
		if (qglesMajorVersion >= 2)
		{
			...
			// GLSL ES 1.00 has no textureCubeLod; fall back to
			// textureCube (mip level 0). Cubemap roughness blurring
			// is lost but the shader compiles.
			if (shaderType == GL_FRAGMENT_SHADER)
			{
				Q_strcat(dest, size, "#define textureCubeLod(a,b,c) textureCube(a,b)\n");
			}
		}
		else
		{
...
void GLSL_InitGPUShaders(void)
{
...
#if defined(__ORBIS__) || defined(__PS4__)
	// ShaccVSH is slow (~5-30s/shader). Disable animation shader variants to cut
	// total compile count: bone animation off (eliminates ~20 variants), GPU vertex
	// animation off so MD3 morphing is CPU-side (eliminates another ~16 variants).
	glRefConfig.glslMaxAnimatedBones = 0;
	glRefConfig.gpuVertexAnimation   = qfalse;
	ri.Printf(PRINT_ALL, "PS4: GPU animation shaders disabled (ShaccVSH compile time reduction)\n");
#endif

	ri.Printf(PRINT_ALL, "Compiling generic shaders (0/%d)...\n", GENERICDEF_COUNT);
	for (i = 0; i < GENERICDEF_COUNT; i++)
	{
		...
		numGenShaders++;
		if ((numGenShaders & 7) == 0)
			ri.Printf(PRINT_ALL, "  generic: %d compiled (%5.2fs elapsed)\n",
			          numGenShaders, (ri.Milliseconds() - startTime) / 1000.0);
	}
	ri.Printf(PRINT_ALL, "generic shaders done: %d (%5.2fs)\n",
	          numGenShaders, (ri.Milliseconds() - startTime) / 1000.0);
	...
	ri.Printf(PRINT_ALL, "fogpass shaders done (%5.2fs)\n", (ri.Milliseconds() - startTime) / 1000.0);
	...
	ri.Printf(PRINT_ALL, "dlight shaders done (%5.2fs)\n", (ri.Milliseconds() - startTime) / 1000.0);
	...
	for (i = 0; i < LIGHTDEF_COUNT; i++)
	{
		...
		numLightShaders++;
		if ((numLightShaders & 7) == 0)
			ri.Printf(PRINT_ALL, "  lightall: %d compiled (%5.2fs elapsed)\n",
			          numLightShaders, (ri.Milliseconds() - startTime) / 1000.0);
	}
	ri.Printf(PRINT_ALL, "lightall shaders done: %d (%5.2fs)\n",
	          numLightShaders, (ri.Milliseconds() - startTime) / 1000.0);
