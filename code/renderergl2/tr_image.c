...
image_t *R_CreateImage2( const char *name, byte *pic, int width, int height, GLenum picFormat, int numMips, imgType_t type, imgFlags_t flags, int internalFormat ) {
	...
	if (qglesMajorVersion)
	{
		...
		case GL_RGBA4:
			...
			break;
		case GL_RGBA16:
			// 16-bit RGBA from DDS/HDR -- downgrade to 8-bit on GLES2
			internalFormat = GL_RGBA;
			dataFormat = GL_RGBA;
			dataType = GL_UNSIGNED_BYTE;
			break;
		...
		case GL_DEPTH_COMPONENT16_ARB:
			...
			break;
		case GL_R32F:
			// Single-channel float not available in GLES2;
			// fall back to luminance (used for hdrDepth)
			internalFormat = GL_LUMINANCE;
			dataFormat = GL_LUMINANCE;
			dataType = GL_UNSIGNED_BYTE;
			break;
	...
	switch(internalFormat)
	{
		...
		case GL_DEPTH_COMPONENT32_ARB:
#endif
			// Fix for sampling depth buffer on old nVidia cards (desktop GL only).
...
void R_CreateBuiltinImages( void ) {
	...
	if (glRefConfig.framebufferObject)
	{
		...
		tr.renderDepthImage  = R_CreateImage("*renderdepth",  NULL, width, height, IMGTYPE_COLORALPHA, IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, GL_DEPTH_COMPONENT16);
		tr.textureDepthImage = R_CreateImage("*texturedepth", NULL, PSHADOW_MAP_SIZE, PSHADOW_MAP_SIZE, IMGTYPE_COLORALPHA, IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, GL_DEPTH_COMPONENT16);
		...
		{
			tr.pshadowMaps[x] = R_CreateImage(va("*shadowmap%i", x), NULL, PSHADOW_MAP_SIZE, PSHADOW_MAP_SIZE, IMGTYPE_COLORALPHA, IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, GL_DEPTH_COMPONENT16);
		...
		if (r_sunlightMode->integer)
		{
			for ( x = 0; x < 4; x++)
			{
				tr.sunShadowDepthImage[x] = R_CreateImage(va("*sunshadowdepth%i", x), NULL, r_shadowMapSize->integer, r_shadowMapSize->integer, IMGTYPE_COLORALPHA, IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, GL_DEPTH_COMPONENT16);
