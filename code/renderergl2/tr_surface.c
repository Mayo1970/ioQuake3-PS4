...
static void RB_SurfaceSprite( void ) {
	...
	rotation = ent->e.data.sprite.rotation;
#else
	radius = ent->e.radius;
...
static void RB_SurfaceOrientedSprite( void )
{
	...
	if (rotation == 0)
	{
		...
		VectorScale( ent->e.axis[2], radius, up );
	}
	else
	{
		float	s, c;
		...
		VectorScale( ent->e.axis[1], c * radius, left );
		VectorMA( left, -s * radius, ent->e.axis[2], left );

		VectorScale( ent->e.axis[2], c * radius, up );
		VectorMA( up, s * radius, ent->e.axis[1], up );
	}
	if ( backEnd.viewParms.isMirror )
		VectorSubtract( vec3_origin, left, left );
...
Shared vertex-stamping helper for the EF line/bezier/electricity primitives.
If corners is non-NULL, the start edge is taken from it (chaining segments)
and the end edge is written back into it for the next call.
==============
*/
static void RB_Line(vec3_t start, vec3_t end, vec3_t linedirection, vec3_t left,
...
static void RB_LineNormal(vec3_t vec1, vec3_t vec2, float scale, vec3_t result)
{
...
A thick line from origin to oldorigin (imod bolts, tractor beams, etc).
==============
*/
...
Tapered variant of RB_SurfaceLine (width differs at each end).
==============
*/
static void RB_SurfaceLine2( void ) {
	refEntity_t *e;
	vec3_t startleft, endleft;
	vec3_t start, end, linedirection, start2origin;
	int ndx, numind;
...
static void RB_SurfaceCylinder(void)
{
	...
	while(index < planes)
	{
		...
		tess.color[ndx][1] = tess.color[ndx+1][1] = tess.color[ndx+2][1] = tess.color[ndx+3][1] = e->shaderRGBA[1] * 257;
		tess.color[ndx][2] = tess.color[ndx+1][2] = tess.color[ndx+2][2] = tess.color[ndx+3][2] = e->shaderRGBA[2] * 257;
		tess.color[ndx][3] = tess.color[ndx+1][3] = tess.color[ndx+2][3] = tess.color[ndx+3][3] = 0xffff;
...
static void RB_SurfaceElectricity(void)
{
